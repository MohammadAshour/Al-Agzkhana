...
function EditInventoryContent() {
  ...
  const familyId = getFamilyId();
  const res = await fetch(`${API_URL}/api/instances/${id}/?family_id=${familyId}`, { headers: await getAuthHeaders() });
  ...
  const res = await fetch(`${API_URL}/api/instances/${id}/?family_id=${familyId}`, {
    method: 'PUT',
    headers: await getAuthHeaders(),
